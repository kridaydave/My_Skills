...
5. **Sequential or parallel?** Steps with a dependency are sequential (design → run → analyze). Independent perspectives on one artifact run parallel (red-team: Quill + Helix + Ethos at once, then synthesize). When the shape of the work matches one of the spawn primitives, route to it instead of hand-assembling the parallel step:
   - Two lenses should *argue it out* over one decision → **`/debate`** (one context, two voices → verdict + flip condition).
